...
st.set_page_config(page_title="AdverSense - Machine Learning-Powered Pharmacovigilance Intelligence", layout="wide")
...
@st.cache_data(show_spinner="Processing FAERS data — first load takes a few minutes")
...
st.markdown(
    """
    <div style="padding: 1.5rem 1rem 1rem 1rem; border-radius: 12px; border: 1px solid #2C2C2C; background: #1A1E29; margin-bottom: 1rem;">
        <h1 style="margin: 0; font-size: 1.75rem; color: #F5F5F5; font-weight: 700;">AdverSense</h1>
        <p style="margin: 0.5rem 0 0 0; color: #A3A3A3; font-size: 0.9rem;">Machine Learning-Powered Pharmacovigilance Intelligence — Transforming adverse event data into explainable clinical intelligence</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    st.caption("All metrics update based on this filter.")
    ...
    if ml_error is not None:
        ...
        with result_col:
            ...
            with st.container(border=True):
                ...
                top_metrics[0].metric("Risk Category", risk, delta="AI Prediction")
                top_metrics[1].metric("Risk Probability", f"{float(rt['prob']) * 100:.1f}%", delta=f"{float(rt['prob']) * 100:.1f}%")
                st.markdown(
                    (
...
st.divider()
st.markdown(
    '<div style="text-align: center; color: #A3A3A3; font-size: 0.85rem; padding: 1rem 0;">'
    '<a href="https://github.com/yourusername/AdverSense" target="_blank" style="color: #00B4D8; text-decoration: none;">AdverSense</a> — '
    'Machine Learning-Powered Pharmacovigilance Intelligence Platform</div>',
    unsafe_allow_html=True,
)
