...
**Note:** Raw FAERS ASCII files are excluded from this repository due to size (each quarterly file exceeds 100MB). See [scripts/download_faers.py](scripts/download_faers.py) for download instructions or use the provided sample dataset for quick testing.
...
AdverSense addresses these challenges by implementing:
...
1. **Clone the Repository**:
   ```bash
   git clone https://github.com/yourusername/AdverSense.git
   cd AdverSense
   ```
2. **Setup Environment**:
...
The project includes an evaluation script to reproduce model performance metrics.

### Quick Test (Sample Data)

Run the ML pipeline on the provided sample dataset (~500 rows) for quick validation:
...
This will:
- Load data from `data/sample/sample_adverse_events.csv`
- Train all 4 models (ANN, Random Forest, Decision Tree, Logistic Regression)
- Output metrics to console and save to `results/metrics_sample.json`

**Note:** The sample dataset is a stratified 3,000-row subset drawn from real 2025 Q4 FAERS data, preserving the ~65/35 serious/non-serious class ratio. Sample metrics will differ slightly from full-data results due to the smaller sample size.

### Full Evaluation (FAERS Data)

To reproduce the full results reported in the README (93.5% recall, 0.74 ROC-AUC):

1. Download FAERS quarterly files (see `scripts/download_faers.py` for instructions)
2. Place the required ASCII files in the project root directory:
   - `DEMO25Q4.txt`
   - `DRUG25Q4.txt`
   - `OUTC25Q4.txt`
   - `REAC25Q4.txt`
...
This will process the full FAERS dataset and output the actual model performance metrics to `results/metrics.json`.
...
- **Threshold**: Optimal classification threshold selected on validation set

**Output Files:**
- `results/metrics.json` - Full FAERS data evaluation results
- `results/metrics_sample.json` - Sample data evaluation results

Both files are saved in JSON format for programmatic access.
